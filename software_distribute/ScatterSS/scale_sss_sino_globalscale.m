...
% REMOVE
%psino = psino.*0.83; 
%psino_tof = psino_tof.*0.83; 
%disp('making psino smaller'); 
...
fname_out = [fdir,'/sss_sino_f',num2str(frame),'_scaled.raw']; 
fid22 = fopen(fname_out,'w'); 
fwrite(fid22,sss_sino,'double'); 
fclose(fid22); 
...
% TOF SSS sino
%sss_sino_tof = zeros(64,13,12,8,8); 
sss_sino_tof = zeros(11,13,12,8,8); 


wt_tof = [0.3 0.6 0.75 0.6 0.3]';
wt_tof = wt_tof./sum(wt_tof); 
	...
	for jj = 1:8
		...
		for kk = 1:13
			for ll = 1:12
				...
				tof_hist_bb = psino_tof(:,kk,ll,ii,jj); 
				tof_hist_bb(tof_hist_bb < 1) = 0; 
				tof_hist_bb = tof_hist_bb(2:end,1); 
				tof_hist_bb = tof_hist_bb(10:53,1); 
				
				tof_hist_bb = reshape(tof_hist_bb,4,11); 
				tof_hist_bb_short = (sum(tof_hist_bb,1))'; 
				tof_hist_bb = tof_hist_bb_short; 
				
				%tof_hist_conv = conv(tof_hist_bb,wt_tof); 
				%tof_hist_conv = tof_hist_conv(3:end-2); 
				%tof_hist_bb = tof_hist_conv; 
				
				tof_hist_bb = tof_hist_bb./(sum(tof_hist_bb(:))); 
				
				sss_temp = sss_sino(kk,ll,ii,jj); 
				sss_tof_temp = tof_hist_bb.*sss_temp; 
				
				sss_sino_tof(:,kk,ll,ii,jj) = sss_tof_temp; 
...
fname_out = [fdir,'/sss_sino_tof_f',num2str(frame),'_scaled.raw']; 
%fname_out = [fdir,'/sss_sino_f',num2str(frame),'_scaled.raw']; 
fid222 = fopen(fname_out,'w'); 
fwrite(fid222,sss_sino_tof,'double'); 
fclose(fid222); 
